...
/**
 * Drives the integration worker without a broker: builds a TaskUpdated event
 * (validated by its schema) and calls processEvent() directly. RabbitMQ will
 * replace this trigger later.
 *
 * Usage: node scripts/emitTaskUpdated.js <taskId> <integrationId>
 */
...
async function main() {
  ...
  if (!Number.isInteger(taskId) || !Number.isInteger(integrationId)) {
    console.error('Usage: node scripts/emitTaskUpdated.js <taskId> <integrationId>');
    process.exit(1);
  }
  ...
  if (!task) {
    console.error(`[emitTaskUpdated] no task with id ${taskId}`);
    process.exit(1);
  }
  ...
  console.log('[emitTaskUpdated] processing', event);
  await processEvent(event);
  console.log('[emitTaskUpdated] done');

  await sequelize.close();
}

main().catch(async (err) => {
  console.error('[emitTaskUpdated] failed:', err);
  await sequelize.close();
  process.exit(1);
});
